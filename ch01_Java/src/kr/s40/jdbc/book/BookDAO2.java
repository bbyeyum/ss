...
public class BookDAO2 {
	...
	public int getStatisReservation(int bk_num) {
		...
		try {
			...
			//SQL문 작성
			sql = "SELECT re_status FROM reservation WHERE re_status=1 AND bk_num=?";
	...
	public void selectMyList(String me_id) {
		...
		try {
			...
			//SQL문 작성
			sql = "SELECT * FROM reservation WHERE me_id = ?";
			...
			if(rs.next()) {
				do {
					System.out.println("도서 대출 번호: " + rs.getInt("re_num"));
					System.out.println("대출 여부: " + rs.getInt("re_status"));
					System.out.println("도서번호: " + rs.getInt("bk_num"));
					System.out.println("회원 아이디: " + rs.getString("me_id"));
					System.out.println("대출 날짜: " + rs.getDate("re_regdate"));
					System.out.println("반납 날짜: " + rs.getDate("re_modifydate"));
				}while(rs.next());
			}else {
				System.out.println("표시할 데이터가 없습니다.");
			}
